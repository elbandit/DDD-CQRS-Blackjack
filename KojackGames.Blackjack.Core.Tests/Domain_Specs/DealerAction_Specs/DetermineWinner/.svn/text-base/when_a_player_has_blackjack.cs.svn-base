...
namespace KojackGames.Blackjack.Core.Tests.Domain_Specs.DealerAction_Specs.DetermineWinner
{
    ...
    public class when_a_player_has_blackjack : with_AnnouceWinnerAction
    {
        private Establish context = () =>
        {
            player = MockRepository.GenerateStub<IPlayer>();
            players_hand = MockRepository.GenerateStub<IPlayersHand>();
            players_hand.Stub(x => x.has_status_of(HandStatus.blackjack)).Return(true);
        ...
        private It should_set_the_players_hand_as_won = () => players_hand.AssertWasCalled(x => x.change_state_to(HandStatus.won));
